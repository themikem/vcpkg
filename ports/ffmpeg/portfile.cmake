...
if("cuda" IN_LIST FEATURES)
    ...
    if(CUDA_FOUND)
    set(ENV{LIB} "$ENV{LIB};${CUDA_LIBRARY_DIRS}")
    set(ENV{INCLUDE} "$ENV{INCLUDE};${CUDA_INCLUDE_DIRS}")
    get_filename_component(NVCC_PATH ${CUDA_NVCC_EXECUTABLE} PATH)
    set(ENV{PATH} "$ENV{PATH};${NVCC_PATH}")
        ...
        message(STATUS "    CUDA npp: ${CUDA_npp_LIBRARY}")
        message(STATUS "    CUDA nvcuvenc: ${CUDA_nvcuvenc_LIBRARY}")
        message(STATUS "    CUDA nvcuvid: ${CUDA_nvcuvid_LIBRARY}")
        message(STATUS "    NVCC EXE: ${CUDA_NVCC_EXECUTABLE}")
        message(STATUS "    ENV LIB: $ENV{LIB}")
        message(STATUS "    ENV INCLUDE: $ENV{INCLUDE}")

        set(OPTIONS "${OPTIONS} --enable-cuda --enable-cuvid --enable-nvenc --enable-libnpp --enable-cuda-sdk")
    endif()
endif()
